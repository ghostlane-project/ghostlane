...
### Android and Linux: calls and games through the tunnel

On Android, and on Linux in system-wide (TUN) mode, the tunnel carried no UDP at all, on every protocol: voice and video calls in Telegram, WhatsApp and Discord, games and anything else on UDP failed while web pages worked. The tunnel now hands UDP to the connection the standard way, as the iPhone app always has.
